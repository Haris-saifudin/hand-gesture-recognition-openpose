...
// <- SVM Declaration
#define PATH_REALTIME_INPUT "./testing/realtime_predict.txt"
#define PATH_OUTPUT "./output.txt"
#define PATH_MODEL_SVM "./dataset.model"
                    ...
                    if (/*(rightWrist_y2 > 0 && rightWrist_x2 > 0) &&*/ leftWrist_y2 > 0 && leftWrist_x2 > 0) {
                        if (/*(rightWrist_y1 < HEIGHT_WINDOWS && rightWrist_x1 < WIDTH_WINDOWS) && */
                            ...
                            if (/*stomatch > rightWrist &&*/ stomatch > leftWrist) {
                                ...
                                loadDataset.open(PATH_REALTIME_INPUT);
                                for (int j = 0; j < 1764; j++) {
                                    if (j == 0) {
                                        loadDataset << 0 << " ";
                                    }
                                    else {
                                        loadDataset << j << ":" << /*tmpData.col(i-1)*/ pose.at<float>(j, 0) << " ";
                                    }
                                }
                                loadDataset.close();

                                // <- load input classification
                                input = fopen(PATH_REALTIME_INPUT, "r");
                                if (input == NULL)
                                {
                                ...
                                //cv::Rect Rec(rightWrist_x2, rightWrist_y2, 80, 80);
                                cv::Rect leftRec(leftWrist_x2, leftWrist_y2, 80, 80);

                                //cv::rectangle(Img, Rec, cv::Scalar(0, 0, 255), 0, 8, 0);
                                cv::rectangle(Img, leftRec, cv::Scalar(0, 0, 255), 0, 8, 0);
                            }
...
int Openpose()
{
    try
    {
        ...
        if ((model = svm_load_model(PATH_MODEL_SVM)) == 0)
        {
